...
fn test_accuracy() {
    ...
    let yf = DayCountConvention::from_int(0)
        .unwrap()
        .yearfrac(start, end);
    ...
    let yf = DayCountConvention::from_int(2)
        .unwrap()
        .yearfrac(start, end);
    assert!((yf - 42.83055555556).abs() < delta);
    let yf = DayCountConvention::from_int(3)
        .unwrap()
        .yearfrac(start, end);
    assert!((yf - 42.24383561644).abs() < delta);
    let yf = DayCountConvention::from_int(4)
        .unwrap()
        .yearfrac(start, end);
    ...
    let yf = DayCountConvention::from_str("act360")
        .unwrap()
        .yearfrac(start, end);
    assert!((yf - 28.78888888889).abs() < delta);
    let yf = DayCountConvention::from_str("act365")
        .unwrap()
        .yearfrac(start, end);
    assert!((yf - 28.39452054795).abs() < delta);
    let yf = DayCountConvention::from_str("eur30/360")
        .unwrap()
        .yearfrac(start, end);
    assert!((yf - 28.37777777778).abs() < delta);
}
